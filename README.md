...
- **Ảnh minh họa**: thứ tự ưu tiên cho mỗi từ:
  ...
  2. Bộ **ảnh SVG vẽ sẵn** đóng gói trong app (`src/assets/words/`) — hiện có đủ 25 từ của
     Unit 1 Session 2. Muốn thêm ảnh cho unit mới: nhờ Claude vẽ thêm file SVG vào thư mục đó
     (tên file = từ viết thường, khoảng trắng thành `-`, ví dụ `be-keen-on.svg`).
